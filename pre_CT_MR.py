...
# convert nii image to npz files, including original image and corresponding masks
modality = "MRI"
anatomy = "Brain_CMB"  # anatomy + dataset name
img_name_suffix = "_space-T2S_desc-masked_T2S.nii.gz"
gt_name_suffix = "_space-T2S_CMB.nii.gz"
prefix = modality + "_" + anatomy + "_"

# Update paths for your dataset
nii_path = "/media/Datacenter_storage/PublicDatasets/cerebral_microbleeds_VALDO/Task2"
gt_path = "/media/Datacenter_storage/PublicDatasets/cerebral_microbleeds_VALDO/Task2"
npy_path = "/media/Datacenter_storage/PublicDatasets/cerebral_microbleeds_VALDO_npy/" + prefix[:-1]
os.makedirs(join(npy_path, "gts"), exist_ok=True)
os.makedirs(join(npy_path, "imgs"), exist_ok=True)

image_size = 1024
voxel_num_thre2d = 10   # Reduced for smaller CMB lesions
voxel_num_thre3d = 50   # Reduced for smaller CMB lesions

# Get subject directories instead of files
subject_dirs = sorted([d for d in os.listdir(nii_path) if d.startswith('sub-')])
print(f"ori # subjects {len(subject_dirs)=}")


# Filter subjects that have both image and ground truth files
valid_subjects = []
for subject in subject_dirs:
    subject_path = join(nii_path, subject)
    if os.path.isdir(subject_path):
        img_file = join(subject_path, subject + img_name_suffix)
        gt_file = join(subject_path, subject + gt_name_suffix)
        if os.path.exists(img_file) and os.path.exists(gt_file):
            valid_subjects.append(subject)

print(f"after sanity check # subjects {len(valid_subjects)=}")
print(valid_subjects)



# For CMB, we typically don't remove specific label IDs
remove_label_ids = []  # Empty for CMB detection
tumor_id = None  # CMBs are small lesions, treat as single class


# MRI doesn't use CT windowing, we'll use percentile normalization
# %% save preprocessed images and masks as npz files
for subject in tqdm(valid_subjects):
    print(f"Processing subject: {subject}")
    
    # Construct file paths
    subject_path = join(nii_path, subject)
    image_name = subject + img_name_suffix
    gt_name = subject + gt_name_suffix
    
    img_file = join(subject_path, image_name)
    gt_file = join(subject_path, gt_name)
    
    # Load ground truth
    gt_sitk = sitk.ReadImage(gt_file)
    gt_data_ori = np.uint8(sitk.GetArrayFromImage(gt_sitk))
    
    # Remove label ids (if any)
    for remove_label_id in remove_label_ids:
        gt_data_ori[gt_data_ori == remove_label_id] = 0
    
    # For CMB detection, you might want to convert all non-zero labels to 1
    # Uncomment the next line if your labels have different values but you want binary masks
    # gt_data_ori[gt_data_ori > 0] = 1
    
    # Handle tumor instances (skip for CMB unless you have multiple CMB classes)
    if tumor_id is not None:
        tumor_bw = np.uint8(gt_data_ori == tumor_id)
        gt_data_ori[tumor_bw > 0] = 0
        tumor_inst, tumor_n = cc3d.connected_components(
    ...
    # Exclude small objects (reduced thresholds for CMB)
    # gt_data_ori = cc3d.dust(
    #     gt_data_ori, threshold=voxel_num_thre3d, connectivity=26, in_place=True
    # )
    ...
    if len(z_index) > 0:
        ...
        # MRI preprocessing (instead of CT windowing)
        if modality == "MRI":
            # Use percentile normalization for MRI
            lower_bound, upper_bound = np.percentile(
                image_data[image_data > 0], 0.5
            ), np.percentile(image_data[image_data > 0], 99.5)
            image_data_pre = np.clip(image_data, lower_bound, upper_bound)
            ...
            # Keep original CT preprocessing if needed
            lower_bound = WINDOW_LEVEL - WINDOW_WIDTH / 2
            upper_bound = WINDOW_LEVEL + WINDOW_WIDTH / 2
            image_data_pre = np.clip(image_data, lower_bound, upper_bound)
            image_data_pre = (
        ...
        output_name = prefix + subject
        np.savez_compressed(
            join(npy_path, output_name + '.npz'), 
            imgs=img_roi, 
            gts=gt_roi, 
            spacing=img_sitk.GetSpacing()
        )
        
        # Save sanity check files
        img_roi_sitk = sitk.GetImageFromArray(img_roi)
        gt_roi_sitk = sitk.GetImageFromArray(gt_roi)
        sitk.WriteImage(
            img_roi_sitk,
            join(npy_path, output_name + "_img.nii.gz"),
        )
        sitk.WriteImage(
            gt_roi_sitk,
            join(npy_path, output_name + "_gt.nii.gz"),
        )
        ...
        for i in range(img_roi.shape[0]):
            ...
            slice_name = output_name + "-" + str(i).zfill(3)
            np.save(
                join(npy_path, "imgs", slice_name + ".npy"),
                resize_img_skimg_01,
            )
            np.save(
                join(npy_path, "gts", slice_name + ".npy"),
                resize_gt_skimg,
            )
    else:
